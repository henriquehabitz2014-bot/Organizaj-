...
function renderDashboard() {
  ...
  if (!htmlHab) htmlHab = '<div class="dash-empty">Nenhum hábito criado ainda <button class="dash-empty-btn" onclick="dashQuickHabito()">+ Hábito</button></div>';
  document.getElementById('dashHabitosHoje').innerHTML = htmlHab;

  // === METAS ===
  var metasAtivas = (estado.metas || []).filter(function(m){ return !m.feito; });
  ...
  if (metasAtivas.length > 0) {
    ...
    metasOrd.slice(0, 3).forEach(function(m){
      ...
      htmlMetas += '<div class="dm-top"><span class="dm-nome">🎯 ' + esc(m.texto) + '</span><span class="dm-pct">' + pct + '%</span></div>';
  ...
  // === LEMBRETES ===
  var lembretesAtivos = (estado.lembretes || []).filter(function(l){ return l.ativo; });
...
function dashQuickNota() {
  ...
  estado.notas.push({titulo:titulo.trim(), texto:texto.trim(), id:uid(), data:new Date().toISOString()});
  salvarEstado();
